...
#  w <- Matrix(w, sparse = TRUE)
  ...
  while  (k<10 && gamma>10^(-40))
  {
    ...
    f_lip = f_y + crossprod(g_w,w-y_w) + 1/2/gamma*norm_vec((w-y_w))^2;
    
    for (m in 1:M)
    {
      
      f_lip = f_lip+crossprod(g_W[,m],W[,m]-y_W[,m])+1/2/gamma*norm_vec((W[,m]-y_W[,m]))^2;
      #####
    }
...
if (type=="ls"){
    for (m in 1:M) {
    #print(m)
#   temp = (2*t(X[as.vector(d==m),]))%*% ((matmult_cpp(X[as.vector(d == m),], (w + W[, m]))-y[d==m]));
temp=crossprod(2 * X[as.vector(d == m),] , matmult_cpp(X[as.vector(d == m),], (w + W[, m])) - y[d == m])
g_w = g_w + temp;
g_W[,m] = g_W[,m] + temp;
}
